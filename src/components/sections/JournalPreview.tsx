...
export default function JournalPreview() {
  ...
  return (
    <section id="journal" className="relative overflow-hidden bg-ink py-28 sm:py-36">
      <div className="relative mx-auto max-w-[1400px] px-5 sm:px-8">
        <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
          <div>
            ...
            <Reveal delay={80}>
              <h2 className="display mt-5 max-w-2xl text-[clamp(2.4rem,5.4vw,4.2rem)] leading-[1.02] text-cream">
                Notes on rest, from
                <span className="gold-text italic"> people who do it daily</span>
              </h2>
            </Reveal>
          </div>
          ...
          <Reveal delay={140}>
            <Link
              href="/blog"
              className="group flex items-center gap-2 rounded-full border border-gold-lt/25 px-6 py-3 text-[11px] uppercase tracking-[0.2em] text-cream transition-all duration-500 hover:border-gold-lt/60 hover:text-gold"
            >
              All articles
              <ArrowRight
                size={14}
              ...
              <Link href={`/blog/${post.slug}`} className="group block h-full">
                <article className="flex h-full flex-col overflow-hidden rounded-[1.6rem] glass transition-all duration-600 hover:-translate-y-2 hover:border-gold-lt/45">
                  ...
                  <div className="flex flex-1 flex-col p-7">
                    ...
                    <h3 className="display mt-3 text-2xl leading-snug text-cream transition-colors duration-400 group-hover:text-gold-lt">
                      {post.title}
                    </h3>
                    ...
                    <span className="mt-6 flex items-center gap-1.5 text-[11px] uppercase tracking-[0.18em] text-gold">
                      Read
                      <ArrowUpRight
                        size={14}
                        strokeWidth={1.5}
