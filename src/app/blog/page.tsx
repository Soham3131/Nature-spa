      ...
      <div className="relative mx-auto max-w-[1400px] px-5 sm:px-8">
        ...
        <Reveal delay={80}>
          <h1 className="display mt-5 max-w-3xl text-[clamp(2.8rem,6.5vw,5rem)] leading-[1.0] text-forest">
            Everything we wish
            <span className="accent-text italic"> guests knew sooner</span>
          </h1>
        </Reveal>
        <Reveal delay={140}>
        ...
        <Reveal delay={200}>
          <Link href={`/blog/${lead.slug}`} className="group mt-16 block">
            <article className="scene-3d grid overflow-hidden rounded-[2rem] glass transition-all duration-600 hover:border-leaf/45 lg:grid-cols-2">
              ...
              <div className="flex flex-col justify-center p-8 sm:p-12">
                <span className="flex items-center gap-3 text-[10.5px] uppercase tracking-[0.2em] text-body/55">
                ...
                <h2 className="display mt-6 text-[clamp(1.9rem,3.4vw,2.9rem)] leading-tight text-forest transition-colors duration-400 group-hover:text-leaf">
                  {lead.title}
                </h2>

                <p className="mt-5 text-[14.5px] leading-[1.8] text-body/78">
                  {lead.excerpt}
                </p>

                <span className="mt-8 flex items-center gap-2 text-[11px] uppercase tracking-[0.2em] text-bronze">
                  Read the article
                  <ArrowUpRight
                    size={15}
                    strokeWidth={1.5}
              ...
              <Link href={`/blog/${post.slug}`} className="group block h-full">
                <article className="flex h-full flex-col overflow-hidden rounded-[1.6rem] glass transition-all duration-600 hover:-translate-y-2 hover:border-leaf/45">
                  ...
                  <div className="flex flex-1 flex-col p-7">
                    ...
                    <h2 className="display mt-3 text-2xl leading-snug text-forest transition-colors duration-400 group-hover:text-leaf">
                      {post.title}
                    </h2>
        ...
        <Reveal>
          <div className="mt-20 overflow-hidden rounded-[2rem] glass p-10 text-center sm:p-14">
            <h2 className="display text-[clamp(1.9rem,4vw,3rem)] leading-tight text-forest">
              ...
              <span className="accent-text italic"> Booking it is better.</span>
            </h2>
            <a
